/* =========================================================
   🛒 MMUST MarketHub - Buyer Dashboard (myorders.js)
...
async function loadOrders() {
  ...
  try {
    ...
    const data = await res.json();
    const orders = data.results || data; // handle both paginated & non-paginated
...
async function trackOrder(orderId) {
  ...
  try {
    const res = await secureFetch(`${API_URLS.trackOrder}${orderId}/`, {
      headers: authHeaders(),
    });
...
async function loadProfile() {
  try {
    const res = await secureFetch(API_URLS.buyerProfile, { headers: authHeaders() });
